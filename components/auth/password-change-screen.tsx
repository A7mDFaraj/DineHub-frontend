...
export function PasswordChangeScreen({ forced = false, expiresAt }: { forced?: boolean; expiresAt?: string | null }) {
  ...
  const [expired] = useState(() => forced && !!expiresAt && Date.parse(expiresAt) <= Date.now());
  ...
  return <main dir="rtl" className="min-h-screen bg-[#130d1b] text-white flex items-center justify-center p-5">
    <section className="w-full max-w-md rounded-3xl border border-white/10 bg-[#201729] p-6 sm:p-8 space-y-5">
      ...
      {done ? <><p role="status">تم حفظ كلمة المرور وإنهاء جميع الجلسات السابقة. سجّل الدخول بكلمتك الجديدة.</p><Link className={input + " block text-center"} href="/admin/login">تسجيل الدخول</Link></> : <>
        ...
        <button disabled={busy} className="min-h-11 underline text-zinc-300" onClick={async () => { try { await signOut(); } finally { router.replace('/admin/login'); router.refresh(); } }}>تسجيل الخروج</button>
      </>}
    </section>
  </main>;
}
