...
interface CartStore {
  ...
  syncPrices: (
    products: { id: string; price: number | string; isAvailable?: boolean }[],
  ) => void;
  items: CartItem[];
  note: string;
  isCartOpen: boolean;
...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  setContext: (context) => {
    if (get().context !== context)
      set({ context, items: [], note: "", isCartOpen: false });
  },
  syncPrices: (products) =>
    set({
      items: get().items.flatMap((item) => {
        const product = products.find(
          (p) => p.id === item.productId && p.isAvailable !== false,
        );
        return product ? [{ ...item, price: Number(product.price) }] : [];
      }),
    }),
  items: [],
  note: "",
        ...
        items: items.map((item, idx) =>
          idx === existingIndex
            ? {
                ...item,
                price: newItem.price,
                quantity: Math.min(99, item.quantity + quantityToAdd),
              }
            : item,
        ),
      });
