...
export default function OrderTrackingPage({
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      let finished = false
      try {
        const res = await apiClient.get(`/orders/${resolvedParams.orderId}`, { signal: controller.signal })
        if (!active) return
        const raw: OrderData = res.data.data || res.data
        setOrder(raw)
        setError("")
        finished = raw.status === "delivered"
        if (raw.trackingPath && raw.trackingPath !== window.location.pathname) router.replace(raw.trackingPath)
      } catch {
        if (active) setError("تعذر تحديث حالة الطلب. تحقق من اتصالك؛ آخر حالة مؤكدة ستبقى ظاهرة.")
      } finally {
        if (active) {
          setLoading(false)
  ...
  if (!loading && !order) return (
    <div dir="rtl" role="alert" className="p-6 rounded-2xl bg-red-50 text-red-800 text-center space-y-4">
      <p>{error || "تعذر العثور على الطلب."}</p>
      <button className="min-h-11 px-5 rounded-xl border focus-visible:outline-2" onClick={() => { setLoading(true); setRetry(value => value + 1) }}>إعادة المحاولة</button>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-4 text-center space-y-2">
        <p className="text-sm font-bold">أظهر هذا الرمز للموظف عند استلام الطلب</p>
        <p dir="ltr" className="font-mono text-3xl font-black tracking-wider">#{shortId}</p>
        {order.createdAt && <p className="text-xs text-stone-600"><OrderElapsed createdAt={order.createdAt} deliveredAt={order.deliveredAt} completed={order.status === "delivered"} /></p>}
      </div>
      ...
      <div className="p-5 rounded-3xl bg-white border border-stone-200 shadow-sm space-y-4">
        ...
        <div className="space-y-4">
          {statusOrder.map((step, index) => {
            ...
            const time = step === "pending" ? order.createdAt : step === "preparing" ? order.acceptedAt : step === "ready" ? order.readyAt : order.deliveredAt
            ...
            return (
              <div key={step} className="flex items-center gap-3.5 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {time && <time dateTime={time} className="text-xs text-stone-500 tabular-nums">{new Date(time).toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" })}</time>}
