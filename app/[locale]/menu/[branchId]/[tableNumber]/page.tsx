...
export default function MenuPage({
  ...
  useEffect(() => {
    const fetchMenuAndTable = async () => {
      try {
        ...
        if (tableRes.status === "fulfilled" && tableRes.value.data) {
          ...
          if (code && code !== resolvedParams.branchId) {
            router.replace(`/menu/${code}/${tableRes.value.data.number}`);
          }
        } else {
          setTableInfo(null);
    ...
    fetchMenuAndTable();
  }, [resolvedParams.branchId, resolvedParams.tableNumber, router, t]);
  ...
  const branchDisplayName = isRtl
    ? tableInfo?.branch?.nameAr || tableInfo?.branch?.name || tableInfo?.branch?.nameEn || "DineHub"
    : tableInfo?.branch?.nameEn || tableInfo?.branch?.name || tableInfo?.branch?.nameAr || "DineHub";
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[65vh] gap-3 text-stone-700">
        ...
        <p className="text-xs text-stone-500 animate-pulse font-bold">{t("loadingMenu")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-32 space-y-5">
      ...
      <header className="p-4 sm:p-5 rounded-3xl bg-white/95 border border-stone-200/80 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] relative overflow-hidden">
        ...
        <div
          className={`absolute -top-16 ${isRtl ? "-left-16" : "-right-16"} w-36 h-36 rounded-full blur-3xl opacity-20 pointer-events-none`}
        ...
        <div className="relative z-10 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {branchLogoUrl ? (
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl overflow-hidden bg-stone-100 border border-stone-200 shrink-0 shadow-sm outline outline-1 -outline-offset-1 outline-black/5">
                <img
                  src={branchLogoUrl}
              ...
              <div
                className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center font-bold text-sm shadow-sm shrink-0 border border-stone-200"
                style={{
            ...
            <div className="min-w-0">
              <h1 className="text-base sm:text-xl font-black text-stone-900 truncate leading-tight">
                {branchDisplayName}
              </h1>
              {branchAddress && (
          ...
          {/* Controls: Language Switcher & Table Badge */}
          <div className="flex items-center gap-2 shrink-0">
            <AdminLanguageSwitcher variant="light" />

            <div
              className="px-2.5 sm:px-3 py-1.5 rounded-2xl border flex flex-col items-center shrink-0 shadow-sm"
              style={{
                backgroundColor: `${branchThemeColor}10`,
                borderColor: `${branchThemeColor}30`,
              }}
            >
              <span className="text-[10px] font-bold text-stone-500">{t("table")}</span>
              <span
                className="text-sm sm:text-base font-black font-mono tabular-nums leading-none"
        ...
        <div className="relative mt-4">
          <Search
            size={16}
            className={`absolute ${isRtl ? "right-3.5" : "left-3.5"} top-1/2 -translate-y-1/2 text-stone-400 pointer-events-none`}
          />
          <input
            type="text"
            ...
            className={`w-full h-11 ${isRtl ? "pr-10 pl-4" : "pl-10 pr-4"} rounded-xl bg-stone-100/90 border border-stone-200/80 text-xs sm:text-sm text-stone-900 placeholder:text-stone-400 focus:bg-white focus:outline-none transition-all box-border`}
            style={{
      ...
      {categories.length > 0 && (
        <div className="sticky top-2 z-30 py-1">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none no-scrollbar -mx-1 px-1">
            <button
              type="button"
              onClick={() => setActiveCategoryId("all")}
              className={`min-h-[36px] px-4 rounded-xl text-xs font-bold transition-all whitespace-nowrap shrink-0 ${
            ...
            {categories.map((cat) => {
              ...
              const catName = isRtl
                ? cat.nameAr || cat.name || cat.nameEn || t("defaultCategory")
                : cat.nameEn || cat.name || cat.nameAr || t("defaultCategory");
      ...
      {filteredCategories.length === 0 ? (
        <div className="p-12 text-center rounded-3xl bg-white border border-stone-200 shadow-sm flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-base font-bold text-stone-900">{t("noMatchingItems")}</p>
              ...
              <section key={category.id} className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3.5">
                  {prods.map((product) => {
                    ...
                    const prodName = isRtl
                      ? product.nameAr || product.name || product.nameEn || t("defaultProduct")
                      : product.nameEn || product.name || product.nameAr || t("defaultProduct");

                    const prodSecondary = isRtl
                      ? (product.nameEn && product.nameEn !== prodName ? product.nameEn : null)
                      : (product.nameAr && product.nameAr !== prodName ? product.nameAr : null);
                      ...
                      >
                        ...
                        {product.imageUrl ? (
                          <div className="w-22 h-22 sm:w-24 sm:h-24 rounded-xl overflow-hidden shrink-0 bg-stone-100 border border-stone-200/60 outline outline-1 -outline-offset-1 outline-black/5 relative shadow-sm">
                            ...
                            {inCart && (
                              <div
                                className={`absolute top-1.5 ${isRtl ? "right-1.5" : "left-1.5"} text-white text-[11px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-md font-mono tabular-nums`}
                                style={{ backgroundColor: branchThemeColor }}
                              >
                                {productCartCount}
                              </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between min-w-0">
                          <div>
                            ...
                            {prodSecondary && (
                              <p className="text-[0.72rem] text-stone-400 mb-1 font-sans truncate" dir={isRtl ? "ltr" : "rtl"}>
                                {prodSecondary}
                              </p>
                            )}
                              ...
                              >
                                {hasAttributes ? (
                                  <>
                                    ...
                                    <span>{inCart ? `${t("customize")} (${productCartCount})` : t("customize")}</span>
                                  </>
                                ) : inCart ? (
                                  <>
                                    <Plus size={13} />
                                    <span>{t("add")} ({productCartCount})</span>
                                  </>
                                ) : (
                                  <>
                                    <Plus size={13} />
